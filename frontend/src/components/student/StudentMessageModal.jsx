...
const StudentMessageModal = ({ userId, onClose, refreshUnreadCount }) => {
    ...
    const [activeConvMenuId, setActiveConvMenuId] = useState(null);
    ...
    // Automatically fetch linked parent(s) and nurses on load
    ...
    // Helper to get unread count for a specific contact from conversations
    const getUnreadCount = (contactUserId) => {
        const conv = conversations.find((c) => String(c.contact_user_id) === String(contactUserId));
        ...
        <div className="student-msg-overlay" onClick={() => { setActiveMenuId(null); setActiveConvMenuId(null); }}>
            <div className="student-msg-modal" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="messenger-container">
                    {/* LEFT SIDEBAR - AUTOMATIC CONTACT LIST */}
                    <div className="messenger-sidebar">
                        <div className="conversations-list">
                            <span className="section-title">My Contacts (Parents & Nurses)</span>
                            {contacts.length === 0 ? (
                                <p className="no-result">No linked contacts found.</p>
                            ) : (
                                contacts.map((contact) => {
                                    const unread = getUnreadCount(contact.user_id);
                                    const isSelected = String(selectedContact?.user_id) === String(contact.user_id);
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="details">
                                                ...
                                                <span className="role-tag">{contact.role.toUpperCase()} ({contact.detail || 'N/A'})</span>
                                            </div>
                                            ...
                                            <div className="conv-action-container">
                                                <button 
                                                    ...
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        setActiveConvMenuId(activeConvMenuId === contact.user_id ? null : contact.user_id);
                                                    }}
                                                >
                                                    <MoreHorizontal size={18} />
                                                </button>

                                                {activeConvMenuId === contact.user_id && (
                                                    <div className="conv-dropdown-menu">
                                                        <button onClick={(e) => handleDeleteConversation(e, contact.user_id)}>
                    ...
                    <div className="messenger-chat-area">
                        {selectedContact ? (
                            ...
                            <div className="empty-chat-placeholder">
                                <User size={48} />
                                <h3>Select your Parent or Nurse to start messaging</h3>
                            </div>
                        )}
                    </div>
                </div>
