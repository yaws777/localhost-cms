...
//#region src/middleware/feature-flag/with-feature-flag.ts
/**
...
* ```
*/
const withFeatureFlag = require_define_middleware.defineMiddleware({
	key: "featureFlag",
	run: (config) => async (req) => {
		const result = await config.evaluate(req);
		const verdict = typeof result === "boolean" ? { enabled: result } : result;
		if (!verdict.enabled) return Response.json(config.rejectBody ?? {
